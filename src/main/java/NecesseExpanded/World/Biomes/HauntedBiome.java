...
public class HauntedBiome extends Biome 
{
    ...
    public void generateRegionSurfaceTerrain(Region region, BiomeGeneratorStack stack, GameRandom random) 
    {
        ...
        stack.startPlaceOnVein(this, region, random, "hauntedReeds").onlyOnWater().customPlace(new GeneratorPlaceFactory.RegionPlaceFunction()
        {
            public void place(GameRandom random, Region region, int regionTileX, int regionTileY, Level level, int tileX, int tileY) {
            if (random.getChance(0.3F) && reeds.canPlace(level, tileX, tileY, 0, false) == null)
            {
    ...
    public void generateRegionCaveTerrain(Region region, BiomeGeneratorStack stack, GameRandom random) {
        ...
        stack.startPlaceOnVein(this, region, random, "hauntedDemonic").onlyOnObject(RockID).placeObjectForced("demonic_ore_haunted_rock");
        stack.startPlace(this, region, random).chance(0.004).placeObject("haunted_rock_small");
        stack.startPlace(this, region, random).chance(0.002).placeObject("haunted_rock_large");
        region.updateLiquidManager();
    }
